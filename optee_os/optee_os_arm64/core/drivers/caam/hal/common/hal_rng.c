...
bool hal_rng_instantiated(vaddr_t baseaddr)
{
	...
	chaVid_ls = read32(baseaddr + CHAVID_LS);
...
enum CAAM_Status hal_rng_kick(vaddr_t baseaddr, uint32_t inc_delay)
{
	 ...
	 */
	val = read32(baseaddr + TRNG_SDCTL);
	...
	write32(val, baseaddr + TRNG_SDCTL);

	/* min. freq. count, equal to 1/4 of the entropy sample length */
	write32(ent_delay >> 2, baseaddr + TRNG_FRQMIN);

	/* max. freq. count, equal to 16 times the entropy sample length */
	write32(ent_delay << 4, baseaddr + TRNG_FRQMAX);

	val = read32(baseaddr + TRNG_MCTL);
	/*
	 ...
	 */
	val = read32(baseaddr + TRNG_MCTL) | TRNG_MCTL_ERR;
	write32(val, baseaddr + TRNG_MCTL);
