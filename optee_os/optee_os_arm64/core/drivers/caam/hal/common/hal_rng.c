...
bool hal_rng_instantiated(vaddr_t baseaddr)
{
	...
	chaVid_ls = get32(baseaddr + CHAVID_LS);
...
enum CAAM_Status hal_rng_kick(vaddr_t baseaddr, uint32_t inc_delay)
{
	...
	put32(baseaddr + TRNG_SDCTL, val);

	/* min. freq. count, equal to 1/4 of the entropy sample length */
	put32(baseaddr + TRNG_FRQMIN, ent_delay >> 2);

	/* max. freq. count, equal to 16 times the entropy sample length */
	put32(baseaddr + TRNG_FRQMAX, ent_delay << 4);

	val = get32(baseaddr + TRNG_MCTL);
	/*
	 ...
	 */
	val = get32(baseaddr + TRNG_MCTL) | TRNG_MCTL_ERR;
	put32(baseaddr + TRNG_MCTL, val);
