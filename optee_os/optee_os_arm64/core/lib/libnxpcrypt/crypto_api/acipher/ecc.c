 ...
 *          ECC crypto_* interface implementation.
 */
/* Global includes */
...
static int algo_isvalid(uint32_t curve, uint32_t algo)
{
	...
	LIB_TRACE("Validate algo 0x%"PRIx32" with curve %d return %d",
			algo, curve, ret);
						...
						size_t size_bits)
{
	...
	LIB_TRACE("ECC Keypair (%d bits) alloc ret = 0x%"PRIx32"",
						size_bits, ret);
	return ret;
}
						...
						size_t size_bits)
{
	...
	LIB_TRACE("ECC Public Key (%d bits) alloc ret = 0x%"PRIx32"",
						size_bits, ret);
	return ret;
}
...
TEE_Result crypto_acipher_gen_ecc_key(struct ecc_keypair *key)
{
	...
	LIB_TRACE("ECC Keypair (%d bits) generate ret = 0x%"PRIx32"",
						size_bits, ret);
					...
					const uint8_t *sig, size_t sig_len)
{
	...
	size_bytes = size_bits / 8;
	if (sig_len != (2 * size_bytes)) {
		LIB_TRACE("Signature length (%d) is invalid expected %d bytes",
					sig_len, (2 * size_bytes));
					...
					void *secret, unsigned long *secret_len)
{
	...
	if (private_key->curve != public_key->curve) {
		LIB_TRACE("Private Key curve (%d) != Public Key curve (%d)",
