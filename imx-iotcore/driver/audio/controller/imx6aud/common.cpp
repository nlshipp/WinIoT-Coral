    ...
    public CUnknown
{
    private:
        ...
        volatile PAUDMUX_REGISTERS       m_pAudMuxRegisters;
...
--*/
{
    ...
    if (m_pAudMuxRegisters != NULL)
    {
        MmUnmapIoSpace(m_pAudMuxRegisters, sizeof(AudMuxRegisters));
    }
...
--*/
{
    ...
    for (index = 0; index < ResourceList->NumberOfEntries(); index++)
    {
        ...
        if (descriptor != NULL)
        {
            if (descriptor->u.Memory.Length >= sizeof(SsiRegisters))
            {
                pSsiRegisters = (PSSI_REGISTERS) MmMapIoSpaceEx(descriptor->u.Memory.Start,
                                                                               sizeof(SsiRegisters),
                                                                               PAGE_READWRITE | PAGE_NOCACHE);
                ASSERT(pSsiRegisters);
                if (pSsiRegisters == NULL)
                {
                    ntStatus = STATUS_INSUFFICIENT_RESOURCES;
                    goto Done;
                }
                m_Soc.InitSsiBlock(pSsiRegisters, interruptDescriptor, m_pPhysicalDeviceObject);
            }
        }
    }
...
--*/
{
    ...
    if (!NT_SUCCESS(ntStatus) || NULL == unknownTopology)
    {
        ...
        // Install IMX6WAV topology miniport for the render endpoint.
        //
        ntStatus = InstallSubdevice(Irp,
                                    MiniportPair->TopoName, // make sure this name matches with IMX6WAV.<TopoName>.szPname in the inf's [Strings] section
                                    CLSID_PortTopology,
                                    CLSID_PortTopology,
    ...
    if (!NT_SUCCESS(ntStatus) || NULL == unknownWave)
    {
        ...
        // Install IMX6WAV wave miniport for the render endpoint.
        //
        ntStatus = InstallSubdevice(Irp,
                                    MiniportPair->WaveName, // make sure this name matches with IMX6WAV.<WaveName>.szPname in the inf's [Strings] section
                                    CLSID_PortWaveRT,
                                    CLSID_PortWaveRT,
