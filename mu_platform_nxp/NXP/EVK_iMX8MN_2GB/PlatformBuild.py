...
class PlatformBuilder(UefiBuilder):
    ...
    def SetPlatformEnv(self):
        ...
        self.env.SetValue("BLD_*_CONFIG_PCIE", "FALSE", "On by default")
        ...
        self.env.SetValue("ACTIVE_PLATFORM", "NXP/EVK_iMX8MN_2GB/EVK_iMX8MN_2GB.dsc", "Platform Hardcoded")
        self.env.SetValue("PRODUCT_NAME", "EVK_iMX8MN_2GB", "Platform Hardcoded")
    ...
    def SetPlatformEnvAfterTarget(self):
        ...
        self.env.SetValue("BUILD_OUTPUT_BASE", value, "Computed in EVK_iMX8MN_2GB Build")

        # Output FD file
        key = "OUTPUT_FD"
        value = os.path.join(self.env.GetValue("BUILD_OUTPUT_BASE"), "FV", "ClientBios.fd")
        self.env.SetValue(key, value, "Computed in EVK_iMX8MN_2GBBuild")

        # Output ROM directory
        key = "OUTPUT_ROM_DIR"
        value = os.path.join(self.env.GetValue("BUILD_OUTPUT_BASE"), "ROM")
        self.env.SetValue(key, value, "Computed in EVK_iMX8MN_2GB Build")
