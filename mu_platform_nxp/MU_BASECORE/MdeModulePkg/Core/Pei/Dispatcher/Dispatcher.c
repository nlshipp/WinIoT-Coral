...
#include "PeiMain.h"
#include <Library/PerformanceLib.h> // MS_CHANGE
...
/**
 * Delayed Dispatch Notify function for EndoPei
 *
 ...
 * @param UniqueId        - Used with WaitOnUniqueId.
...
PeiDelayedDispatchRegister (
    ...
    if (0 == Delay) {   // Force early dispatch point
...
/**
 * Delayed Dispatch Wait for a particular entry. Insure that any dispatch entry that contains
 * the UniqueId is complete before returning.
 *
 * @param[in] This          - Pointer to DelayedDispatch PPI.
 * @param[in] UniqueId      - Pointer to UniqueId Guid.
 *
 * @return EFI_STATUS EFIAPI
 */
EFI_STATUS
...
//MS_CHANGE Delayed Dispatch end
  ...
  )
{
  ...
  if (NULL == Private->DelayedDispatchTable) {                            // MSCHANGE Start
    ...
    if (NULL != GuidHob) {
      ...
      UINT32 TableSize = sizeof(DELAYED_DISPATCH_TABLE) + ((FixedPcdGet32(PcdDelayedDispatchMaxEntries) - 1) * sizeof(DELAYED_DISPATCH_ENTRY));
    ...
    for (Index1 = 0; Index1 < Private->FvCount; Index1++) {
      for (Index2 = 0; Index2 < Private->Fv[Index1].PeimCount; Index2++) {
        if (Private->Fv[Index1].PeimState[Index2] == PEIM_STATE_REGISTER_FOR_SHADOW) {
          ...
          PERF_LOAD_IMAGE_BEGIN (NULL); // MS_CHANGE
          Status = PeiLoadImage (
          ...
          if (Status == EFI_SUCCESS) {
            PERF_LOAD_IMAGE_END (PeimFileHandle); // MS_CHANGE
            //
        ...
        if (Private->Fv[FvCount].PeimState[PeimCount] == PEIM_STATE_NOT_DISPATCHED) {
          if (!DepexSatisfied (Private, PeimFileHandle, PeimCount)) {
            ...
            if ((Private->PeiMemoryInstalled) && (Private->Fv[FvCount].PeimState[PeimCount] == PEIM_STATE_REGISTER_FOR_SHADOW) &&   \
              ...
              // PERF_ENTRY_POINT_BEGIN (PeimFileHandle); // MS_CHANGE
              PeimEntryPoint (PeimFileHandle, (const EFI_PEI_SERVICES **) PeiServices);
              //PERF_END (PeiServices, L"PEIM", PeimFileHandle, 0); // MS_CHANGE
              // PERF_ENTRY_POINT_END (PeimFileHandle); // MS_CHANGE
